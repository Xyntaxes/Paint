...
void draw() {
  ...
  rect(0,0,165,1000);
   ...
   text("Clear",113,485);
   textSize(15);
   text("Have fun painting",10,570);
   text("Press S to save!",10,590);
   
   if (keyPressed){      // Save function
     if(key == 's' | key == 'S') {
       save = save + 1;
       fill(255);
       stroke(255);
 ...
 if (erase % 2 == 0) {                  //Erase function
  ...
  else if(mouseX>xy1 && mouseX <xy1+xyz1 && mouseY>xy2 && mouseY <xy2+xyz2){    //Fill
    fill(one);
    rect (165,0,1400,1000);
    rainbow = false;
  }
